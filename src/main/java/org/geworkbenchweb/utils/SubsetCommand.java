 ...
 * 
 */
/* TODO This is based on a class in hierarchical clustering package. That one probably would be better to be merged with this. */
public abstract class SubsetCommand implements Command {
	
	protected abstract List<String> getItems(); /* this depends how the items are got, 
	considering the items are usually not set when this class is instantiated, but when the command is run ('menuSelected'). */
