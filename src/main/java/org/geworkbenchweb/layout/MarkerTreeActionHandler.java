...
public class MarkerTreeActionHandler extends TreeActionHandler {
	...
	private void addMarkerSetAction(final Tree sender) {
		...
		if (mark.equalsIgnoreCase("[]") || mark.equalsIgnoreCase("[markers]")) {
			MessageBox.createInfo().withCaption("Warning").withMessage("There is no marker selected!").withOkButton()
					.open();
			return;
		}
			...
			public void buttonClick(ClickEvent event) {
				try {
					if (setName.getValue() != null) {
						...
						for (final SubSet markerset : markersets) {
							...
							if (name.equals(setName.getValue())) {
								...
								MessageBox.createInfo().withCaption("Warning")
										.withMessage("There is a Marker Subset with the name \"" +
												name +
										...
										.withOkButton(() -> {
											...
											for (int i = 0; i < temp.length; i++) {
												String data = (String) sender.getItem(Integer.parseInt(temp[i].trim()))
											...
											if (newmarkers.size() > 0) {
												...
												markerSetTree.getContainerProperty(markerset.getId(),
														SetViewLayout.SUBSET_NAME).setValue(name1);
												markerSetTree
														.getContainerProperty(markerset.getId(),
																SetViewLayout.SET_DISPLAY_NAME)
														.setValue(name1 + " [" + markers.size() + "]");
												for (int j = 0; j < newmarkers.size(); j++) {
													markerSetTree.addItem(newmarkers.get(j) + markerset.getId());
													markerSetTree
															.getContainerProperty(newmarkers.get(j) + markerset.getId(),
																	SetViewLayout.SET_DISPLAY_NAME)
															.setValue(newmarkersWithGenename.get(j));
													markerSetTree.setParent(newmarkers.get(j) + markerset.getId(),
															markerset.getId());
													markerSetTree.setChildrenAllowed(
															newmarkers.get(j) + markerset.getId(), false);
												}
											}
						...
						Long subSetId = SubSetOperations.storeMarkerSetInContext(markers, subSetName, dataSetId,
								context);
						markerSetTree.addItem(subSetId);
						markerSetTree.getContainerProperty(subSetId, SetViewLayout.SUBSET_NAME).setValue(subSetName);
						markerSetTree.getContainerProperty(subSetId, SetViewLayout.SET_DISPLAY_NAME)
								.setValue(subSetName + " [" + markers.size() + "]");
						...
						for (int j = 0; j < markers.size(); j++) {
							...
							markerSetTree
									.getContainerProperty(markers.get(j) + subSetId, SetViewLayout.SET_DISPLAY_NAME)
									.setValue(markersWithGenename.get(j));
