...
public class QuickIntro extends VerticalLayout {
	...
	public void attach() {
		...
		DownloadStream downloadStream = new ClassResource("Intro.html",
				getApplication()).getStream();
		...
		/* note this is not a full-blown HTML parser, only handling simple text and img tag */
		...
		while(matcher.find()) {
			...
			Label label = new Label(text.substring(index, imgBegin), Label.CONTENT_XHTML);
			addComponent(label);
