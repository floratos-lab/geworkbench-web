			...
			public void menuSelected(MenuItem selectedItem) {
				if (GeworkbenchRoot.genespaceEnabled()) {
					if (UMainToolBar.this.layout == null) {
						...
						UMainToolBar.this.layout = new GenspaceLayout(
								genSpaceLogger, pusher);

						if (!UMainToolBar.this.layout.getGenSpaceLogin_1()
								.autoLogin(username, password, true)) {
							UMainToolBar.this.layout.getGenSpaceLogin_1()
									.authorizeLayout();
						}
					}
			...
			public void menuSelected(MenuItem selectedItem) {
				...
				if (GeworkbenchRoot.genespaceEnabled()) {
					if (!genSpaceServerFactory.userLogin(username, password)) {
						...
						genSpaceLogger.getGenSpaceLogin().autoLogin(username,
								password, false);
