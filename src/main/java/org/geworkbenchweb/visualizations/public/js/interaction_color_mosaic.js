...
$interaction_color_mosaic.create = function(id, interactome, gene_symbol,
	...
	for (var i = 0; i < gene_symbol.length; i++) {
		...
		var row = "<td width=100 height=10><a href='http://www.genecards.org/cgi-bin/carddisp.pl?gene="
				+ gene_symbol[i]
				+ "' target='_blank'>"
				+ gene_symbol[i]
				+ '</a></td>';
		for (var j = 0; j < interactome.length; j++) {
			...
			row += '<td width=100 height=10 bgcolor=' + color[index] + " style='color:white'>" + p_formatted + '</td>';
		}
