...
public class CNKBUI extends VerticalLayout implements AnalysisUI {
	...
	static boolean isExcludedInteractome(String interactomeAndCount) {
		...
		if (excludedInteractomes.contains(interactome))
			return true;
		for (String prefix : excludedPrefixes) {
			if (interactome.startsWith(prefix))
				return true;
		}
		return false;
	}
	...
	public void attach() {
		...
		try {
			...
			for (String interactome : interactonList) {
				if (isExcludedInteractome(interactome))
					continue;
			...
			public void valueChange(Property.ValueChangeEvent valueChangeEvent) {
				...
				try {
					...
					if (object instanceof InteractomeAndVersion) {
						...
						if (obj instanceof InteractomeAndVersion)
							a = (InteractomeAndVersion) obj;
						...
						try {
							...
							if (dataSetAnnotation == null) {
								MessageBox.createWarning().withCaption("Warning")
										.withMessage("No annotation file was loaded, cannot query CNKB.").withOkButton()
										.open();
								return;
							}
							...
							// Define an array of 4 Warning messages based on priorities for each situation
							// below
							...
							if (obj == null || !(obj instanceof Set<?>)) { // this never happens for multi-select
								...
								if (interactomes.size() == 0) {
									...
									for (InteractomeAndVersion a : interactomes) {
										if (!a.interactome.startsWith("aracne_")
												&& !a.interactome.startsWith("cindy_")) {
											priorityWarningMessages[2] = a.interactome
							...
							if (warningMesaage != null) {
								MessageBox.createWarning().withCaption("Warning").withMessage(warningMesaage)
										.withOkButton().open();
								return;
							}
							String version = null; /*
													 * only one version is allowed even if multiple interactome is
													 * selected.
													 */
		...
		sourceSelector.addValueChangeListener(new Property.ValueChangeListener() {
	...
	private CNKBResultSet getInteractions(Long dataSetId,
		...
		Map<String, AnnotationEntry> annotationMap = new HashMap<String, AnnotationEntry>(); // TODO this may be more
																								// efficient by using
																								// JPA directly
		...
		for (String context : allInteractiomesSelected) {
			for (String marker : selectedMarkers) {
				...
				if (geneId != null && !geneId.trim().equals("---")) {
					/*
					 * the earlier codes says to do this only when it is 'dirty'. it does not appear
					 * correct.
					 */
				...
				/*
				 * FIXME update preference. this is inherited from the earlier code. I don't
				 * think it does the correct thing
				...
				if (interactionDetails != null && interactionDetails.size() > 0) {
					...
					if (cnkbPref.getSelectedConfidenceType() == null
						...
						cnkbPref.setSelectedConfidenceType(cnkbPref.getConfidenceTypeList().get(0)); // use first one as
																										// default
																										// value.
				}
	...
	public String execute(Long resultId,
		...
		try {
			...
			e.printStackTrace();
			throw new IOException("null pointer caught in CNKBUI" + e); // using IOException because of the limitation
																		// of the interface AnalysisUI
		}
		return "CNKB";
	}
