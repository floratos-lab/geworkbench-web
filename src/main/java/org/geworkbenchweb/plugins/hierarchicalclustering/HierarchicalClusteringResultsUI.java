...
public class HierarchicalClusteringResultsUI extends VerticalSplitPanel implements Visualizer {
	...
	public HierarchicalClusteringResultsUI(Long dataSetId) {
		...
		dispPref.addItem("Array positioning",
				new DisplayOptionCommand("Array Positioning", this, DisplayOptionCommand.SetType.MICROARRAY,
						dendrogram));
	 ...
	 * result stored in a StringBuffer buffer, so buffer should be set to empty
	 * before starting from the root
	 */
	...
	private Range[] updateRange(final int[] selectedMarkers,
		...
		if (selectedArrays.length == 1) {
			 ...
			 * calculate differently (over all markers, only once) and use it for all
			 * markers
			 */
	...
	/*
	 * the name 'Range' is a left-over from earlier code of desktop version. just
	 * two statistics: mean and standard deviation
	 */
