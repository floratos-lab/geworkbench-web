window.org_geworkbenchweb_visualizations_Cytoscape = function () {
	this.onStateChange = function () {
		$network_viewer.create(this.getElement(), this.getState().nodes, this.getState().edges, this.getState().layoutName)
		if ("colors" in this.getState())
			$network_viewer.set_color(this.getState().colors)
	}
	if (this.getElement().id === "") {
		this.getElement().id = "gw-cytoscape"
	}
}

var $network_viewer = {}; /* module namespace */

$network_viewer.create = function (element, nodeArray, edgeArray, layout) {
	...
	cy.on('cxttap', 'node', function () {
		$.contextMenu('destroy', "#gw-cytoscape");
		var sym = this.data('id');
		$.contextMenu({
			selector: "#gw-cytoscape",
